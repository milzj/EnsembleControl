...
def _draw_ci_errorbar(ax, cis, level, value_label):
    # Drawn UNLABELED: a labelled errorbar() hands the legend an ErrorbarContainer,
    # which HandlerErrorbar renders as marker + line + a capped error bar inside the
    # key.  The subsampling _ci{level} figure labels a plain Line2D and gets a short
    # line + marker instead, so the two legends disagreed for the same quantity.  The
    # caller supplies a Line2D proxy for the J_hat_N* entry to match it.
...
def plot_plugin(run_or_path, outdir=None, prefix="plugin", stamp=None,
    ...
    for level in levels:
        ...
        # Proxy for the \widehat J_N^* series: the errorbar is drawn unlabeled, so the
        # key is a plain line + marker (HandlerLine2D), the same key the subsampling
        # _ci{level} figure gets from its labelled ax.plot.  lw/markersize come from the
        # rcParams configure_style() just set, so both keys render identically.
        jhat = mlines.Line2D([], [], color="C0", marker="o", ls="-",
                             label=r"$\widehat J_N^*$")
        beta = mpatches.Patch(color="none",
                              label=r"$\beta = {:g}$".format(1.0 - level))
        ax.legend(handles=_params([jhat]) + [beta])
        fig.tight_layout()
        if saving:
